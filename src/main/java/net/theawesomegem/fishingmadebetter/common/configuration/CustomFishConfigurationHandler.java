...
public class CustomFishConfigurationHandler {
    ...
    public static boolean loadFishes() {
        ...
        for (File configFile : configFiles) {
            if (!configFile.exists() || !configFile.getName().endsWith(".json")) {
                continue;
            }
    ...
    private static void loadFishData(JsonElement jsonElement) {
        ...
        for (Map.Entry<String, JsonElement> fishDataEntry : fishDataJson.entrySet()) {
            ...
            fishDataMap.put(fishDataEntry.getKey(), fishData);
        }

        CommonProxy.Logger.log(Level.INFO, "=========== Loaded Fishes ============");

        for (FishData fishData : fishDataMap.values()) {
            CommonProxy.Logger.log(Level.INFO, fishData.fishId);
        }
