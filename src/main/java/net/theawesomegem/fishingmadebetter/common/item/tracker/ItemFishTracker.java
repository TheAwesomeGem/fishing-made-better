...
public abstract class ItemFishTracker extends Item {
    ...
    public ActionResult<ItemStack> onItemRightClick(World worldIn, EntityPlayer playerIn, EnumHand handIn) {
        ...
        if(trackingLiquid.equals(FishingLiquid.VOID)) {
        	if(pos.getY() > 3) {
        		playerIn.sendMessage(new TextComponentTranslation("notif.fishingmadebetter.fish_tracker.only_void"));
        		return new ActionResult<ItemStack>(EnumActionResult.FAIL, itemstack);
        	}
        }
        else if(trackingLiquid.equals(FishingLiquid.WATER)) {
        	if(blockClicked.getMaterial() != Material.WATER) {
        		playerIn.sendMessage(new TextComponentTranslation("notif.fishingmadebetter.fish_tracker.only_water"));
        		return new ActionResult<ItemStack>(EnumActionResult.FAIL, itemstack);
        	}
        }
        else if(trackingLiquid.equals(FishingLiquid.LAVA)) {
        	if(blockClicked.getMaterial() != Material.LAVA) {
        		playerIn.sendMessage(new TextComponentTranslation("notif.fishingmadebetter.fish_tracker.only_lava"));
        		return new ActionResult<ItemStack>(EnumActionResult.FAIL, itemstack);
        	}
        } 
        ...
        if(blockClicked.getMaterial() == MaterialLiquid.WATER) {
        	...
        	if(waterCount < 25) {
        		playerIn.sendMessage(new TextComponentTranslation("notif.fishingmadebetter.fish_tracker.too_shallow"));
        		return new ActionResult<ItemStack>(EnumActionResult.FAIL, itemstack);
        	}
        }
        ...
        playerIn.sendMessage(new TextComponentTranslation("notif.fishingmadebetter.fish_tracker.tracking_start"));
